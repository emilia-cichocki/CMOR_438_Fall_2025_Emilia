...
The Perceptron algorithm begins by randomly initializing a vector of weights $\mathbf{w} = [w_1, ... w_k]$ with length $k$ (number of features) and a bias term. For a sample with an associated feature vector 

$$
\mathbf{x}_i = [x_{i1}, ..., x_{ik}]^\top
$$

the Perceptron calculates the pre-activation value 
...
Once the Perceptron has been trained on a training dataset to find optimal weights and biases, it can be used to predict the class of a previously unseen sample. For every test sample with a feature vector 
