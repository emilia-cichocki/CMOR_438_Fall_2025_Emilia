...
### Initialization
An MLP contains $L$ hidden layers of various sizes, not including the input layer ($l = 0$) and a final output layer ($l = L + 1$). The MLP algorithm begins by randomly initializing $L + 1$ weight matrices $\mathbf{W}^l$ connecting layer $l$ to layer $l+1$ for $l = 0, ... , L$. Each matrix has a dimension ($n_{l}$, $n_{l+1}$), where $n_{i}$ is the number of neurons in layer $i$. Additionally, it initializes $L + 1$ bias vectors $b^l$, each of which is a zero vector with $n_{l+1}$ elements. For a sample with an associated feature vector $\mathbf{x}_i = [x_{i1}, ..., x_{ik}]$ with $k = n_0$, the MLP first sets the input layer vector to be $\mathbf{a}_i^0 = \mathbf{x}_i$. 
...
$$
where $n$ is the number of samples, $\mathbf{a}_i^{L + 1}$ is the final post-activation output, and $\mathbf{y}_i$ is the encoded true targets. The gradient for a single sample is given by
$$
...
Once the MLP has been trained on a training dataset to find optimal weights and biases, it can be used to predict the class of a previously unseen sample. For every test sample with a feature vector $\mathbf{x}_i = [x_{i1}, ..., x_{ik}]^\top$, the class probabilities are calculated by performing a feedforward step using the weights, biases, and feature vector. The final post-activation value or vector is taken as the output. For binary classification problems, the sample is labeled with the positive class if this value is greater than 0.5; in multi-class problems, the class with the highest probability is selected.
...
6. *Log Loss*: Cross-entropy loss for predicted class probabilities that accounts for the relative confidence of a prediction; for $\hat{y}_{i, y_i}$ the predicted probability for a true class $y_i$ and sample $i$, given by 
$$
